(function() {
	...
	Game.prototype.allObjects = function() {
		return [].concat(this.bullets, this.asteroids, this.ship);
	};
	...
	Game.prototype.drawPause = function(ctx) {
		...
		if(key.isPressed('p')) this.gameView.start();
	};
	...
	Game.prototype.drawIntro = function(ctx) {
		...
		ctx.fillRect(Game.DIM_X / 2, Game.DIM_Y / 2 - 20, 560, 200);
		ctx.fillRect(Game.DIM_X / 2, Game.DIM_Y / 2 - 20, 560, -75);
		ctx.fillRect(Game.DIM_X / 2, Game.DIM_Y / 2 - 20, -560, 200);
		ctx.fillRect(Game.DIM_X / 2, Game.DIM_Y / 2 - 20, -560, -75);
		ctx.globalAlpha = 1;
		...
		if(this.pulse >= Asteroids.Game.FPS) {
			...
			ctx.fillText('Press <enter> to Start', Game.DIM_X / 2, Game.DIM_Y / 2 + 65);
			this.pulse += 1;
		}

		ctx.font = '20px gameFont';
		ctx.fillText(
			'Controls: ↑↓←→ or w-a-s-d to move, space to shoot',
			Game.DIM_X / 2, Game.DIM_Y / 2 + 140
		);

		ctx.fillStyle = '#000';
		ctx.fillRect(Game.DIM_X / 2 - 275, Game.DIM_Y - 40, 550, 40);
		ctx.fillStyle = '#FFF';
		ctx.font      = '25px Arial';
		ctx.fillText(
			'Source Code: github.com/dwightware4/Asteroids',
			Game.DIM_X / 2, Game.DIM_Y - 15
		);

		if(key.isPressed('enter')) this.gameView.start();
	};
	...
	Game.prototype.drawOutro = function(ctx) {
		...
		ctx.fillStyle   = '#000';
		ctx.globalAlpha = 0.65;
		ctx.fillRect(Game.DIM_X / 2, Game.DIM_Y / 2 - 20, 425, 200);
		ctx.fillRect(Game.DIM_X / 2, Game.DIM_Y / 2 - 20, 425, -75);
		ctx.fillRect(Game.DIM_X / 2, Game.DIM_Y / 2 - 20, -425, 200);
		ctx.fillRect(Game.DIM_X / 2, Game.DIM_Y / 2 - 20, -425, -75);
		ctx.globalAlpha = 1;
		...
		if(key.isPressed('enter')) this.gameView.reset();
	};
